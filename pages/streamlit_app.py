...
with tabs[2]:
    ...
    overhang_m = (provided_footing_side_m - column_width_mm / 1000) / 2  # m
    net_upward_pressure_moment = factored_load / (provided_footing_side_m ** 2) # Using factored load for moment calculation
    Mx_kNm = 0.5 * net_upward_pressure_moment * (overhang_m ** 2) * provided_footing_side_m / 1000 # kNm (Total moment)
    My_kNm = Mx_kNm # Same due to square footing symmetry
    moment_per_meter = Mx_kNm / provided_footing_side_m * 1000 # kNm/m

    st.write(f"Provided Footing Size: {provided_footing_side_mm:.0f} mm × {provided_footing_side_mm:.0f} mm")
    st.write(f"Net upward pressure (for moment): {net_upward_pressure_moment:.2f} kN/m²")
    st.write(f"Factored moment (Mx = My): {moment_per_meter:.2f} kNm/m")  # Corrected unit display
    ...
    K = moment_per_meter * 1e6 / (fcd_Nmm2 * 1000 * effective_depth_mm ** 2)  # b = 1000 mm for per meter width
    ...
    As_required = (moment_per_meter * 1e6) / (0.87 * fy_Nmm2 * la)  # mm²/m (Required steel area per meter width)
...
with tabs[3]:
    ...
    column_width_m = column_width_mm / 1000
    column_length_m = column_length_mm / 1000
...
with tabs[4]:
    ...
    provided_side_mm_3d = st.number_input("Footing Side Length for 3D View (mm)",
                                         value=int(provided_footing_side_m),
    ...
    fig3d.update_layout(
        ...
        scene=dict(
            xaxis=dict(title=f'{provided_footing_side_mm:.0f} (mm)', range=[0, provided_side_mm_3d]),  # Added range for better view
            yaxis=dict(title=f'{provided_footing_side_mm:.0f} (mm)', range=[0, provided_side_mm_3d]),
            zaxis=dict(title=f'{footing_depth} (mm)', range=[0, 300]),  # Added range for Z
